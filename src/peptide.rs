...
impl Peptide {
    ...
    /// Gives the formula for the whole peptide. With the global isotope modifications applied, and with the N and C terminal modifications.
    pub fn formula(&self) -> Option<MolecularFormula> {
        self.bare_formula()
            .map(|f| f + self.n_term() + self.c_term())
    }

    /// Get the formula of this peptide without N and C terminal modifications (or H2O if there are no modifications). With the global isotope modifications applied.
...
impl SequenceElement {
    ...
    pub fn modified(
        ...
        Self {
            aminoacid,
            modifications: modifications.to_owned(),
            possible_modifications: Vec::new(),
            ambiguous,
        }
    }
